...
fn download_pretrained_weights(cache_path: &PathBuf, vgg_version: &VggVersion) {
    if !cache_path.exists() {
        ...
        let bytes =
            download_file_as_bytes(url, "Downloading VGG pretrained weights from PyTorch...");
...
fn get_vgg_config_array(vgg_version: &VggVersion) -> &[i32] {
    match vgg_version {
        VggVersion::Vgg11 | VggVersion::Vgg11Bn => {
            &[64, -1, 128, -1, 256, 256, -1, 512, 512, -1, 512, 512, -1]
        }
        VggVersion::Vgg13 | VggVersion::Vgg13Bn => &[
            64, 64, -1, 128, 128, -1, 256, 256, -1, 512, 512, -1, 512, 512, -1,
        ],
        VggVersion::Vgg16 | VggVersion::Vgg16Bn => &[
            64, 64, -1, 128, 128, -1, 256, 256, 256, -1, 512, 512, 512, -1, 512, 512, 512, -1,
        ],
        VggVersion::Vgg19 | VggVersion::Vgg19Bn => &[
            64, 64, -1, 128, 128, -1, 256, 256, 256, 256, -1, 512, 512, 512, 512, -1, 512, 512,
            512, 512, -1,
        ],
    }
}
...
fn is_batch_norm_vgg(vgg_version: &VggVersion) -> bool {
    match vgg_version {
        VggVersion::Vgg11Bn | VggVersion::Vgg13Bn | VggVersion::Vgg16Bn | VggVersion::Vgg19Bn => {
            true
        }
        _ => false,
    }
    ...
    for num in vgg_config_array {
        if *num == -1 {
            ...
            key_patterns.push((
                ...
                format!(
                    "conv_block{}.conv_layers.{}.weight",
                    burn_block_index, burn_layer_index
                ),
            ));
            key_patterns.push((
                format!(r"^features\.{}\.bias$", pytorch_index),
                format!(
                    "conv_block{}.conv_layers.{}.bias",
                    burn_block_index, burn_layer_index
                ),
            ));
            ...
            if is_batch_norm_vgg {
                ...
                key_patterns.push((
                    ...
                    format!(
                        "conv_block{}.bn_layers.{}.running_mean",
                        burn_block_index, burn_layer_index
                    ),
                ));
                key_patterns.push((
                    ...
                    format!(
                        "conv_block{}.bn_layers.{}.running_var",
                        burn_block_index, burn_layer_index
                    ),
                ));
            }
...
pub fn load_pretrained_weights<B: Backend>(mut vgg: Vgg<B>, vgg_version: VggVersion) -> Vgg<B> {
    ...
    let mut store = PytorchStore::from_file(cache_path).map_indices_contiguous(false);
    ...
    let result = vgg
        .load_from(&mut store)
        .expect("Failed to load pretrained VGG weights");
    println!("\n{}", result);
